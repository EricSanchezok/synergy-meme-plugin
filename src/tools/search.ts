...
function scoreTemplate(template: MemeTemplate, query: string) {
  if (!query) return 1
...
export const searchMemeTemplates = tool({
  ...
  args: {
    ...
    lineCount: tool.schema.number().int().min(1).max(8).optional().describe("Only return templates with this line count."),
    style: tool.schema.string().optional().describe("Only return templates supporting this style."),
  },
  async execute(args) {
    const limit = args.limit ?? 12
    const style = args.style ? normalize(args.style) : undefined
    const matches = templates
      .filter((template) => (args.lineCount ? template.lines === args.lineCount : true))
      .filter((template) => (style ? template.styles.map(normalize).includes(style) : true))
      .map((template) => ({ template, score: scoreTemplate(template, args.query ?? "") }))
      .filter((entry) => !args.query || entry.score > 0)
