...
`publish:market` uses `synergy-plugin publish-market --repo https://github.com/EricSanchezok/synergy-meme-plugin`. It validates the stable Plugin API 4 `definePlugin()` descriptor, builds and packs the generated artifact, signs and uploads GitHub Release assets when `gh` is authenticated, writes the registry v2 entry, regenerates `registry.json`, and opens a PR against `SII-Holos/synergy-plugins`.
