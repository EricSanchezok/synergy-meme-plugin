...
`publish:market` uses `synergy-plugin publish-market --repo https://github.com/EricSanchezok/synergy-meme-plugin`. It validates, builds, packs, signs, uploads GitHub Release assets when `gh` is authenticated, writes the official registry entry including `compatibility.synergy` from `plugin.json` `engines.synergy`, regenerates `registry.json`, and opens a PR against `SII-Holos/synergy-plugins`.
